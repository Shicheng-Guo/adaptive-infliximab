...
# Function for creating a data frame ready for mrgsolve simulation
	label.function <- function(induction.data) {
...
# Simulate the maintenance phase from "induction.data" using "label.function" for each ID (individual) in each SIM (simulation group)
	label.data <- ddply(induction.data, .(ID,SIM), label.function)

# ------------------------------------------------------------------------------
# Write label.data to a .csv file
	label.filename <- paste0("time_dep_",time.dep,"_label.csv")
	write.csv(label.data,file = label.filename,na = ".",quote = F,row.names = F)
