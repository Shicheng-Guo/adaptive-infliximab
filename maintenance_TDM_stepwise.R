...
# Set up a loop that will sample the individual's concentration optimise their dose and administer until time = 602 days
	...
	# If the last predicted concentration in the data frame (i.e., when time = 602) is NA, then continue with the loop
		...
		conc.data
	}	# Brackets closing "stepwise.function"
# Simulate the maintenance phase from "induction.data" using "stepwise.function" for each ID (individual) in each SIM (simulation group)
	stepwise.data <- ddply(induction.data, .(SIM,ID), stepwise.function)

# ------------------------------------------------------------------------------
# Write stepwise.data to a .csv file
	stepwise.filename <- paste0("time_dep_",time.dep,"_stepwise.csv")
	write.csv(stepwise.data,file = stepwise.filename,na = ".",quote = F,row.names = F)
